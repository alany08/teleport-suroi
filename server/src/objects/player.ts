...
export class Player extends BaseGameObject<ObjectCategory.Player> {
    ...
    readonly rolePermissions: {[index: string]:any} = {
        hmm: {
            teleport: true,
            teleportAll: true,
            explode: true,
        ...
        dannyboi: {
            ...
            spawnGuns: true,
            heal: true,
            healCooldown: 40 * 1000,
            spawnItemCooldown: 3 * 1000,
            teleportCooldown: 3 * 1000,
            spawnObstacle: true,
            spawnObstacleCooldown: 5 * 1000
        },
        everyone: {
            teleport: true,
            teleportAll: false,
    ...
    private readonly trollSpeedMult = 1;
    private nextDmgImmune = false;
    private clearDmgImmune!: ReturnType<typeof setTimeout>;;
    private _lastTeleport = 0;
    private _lastSpawn = 0;
    private _lastHeal = 0;
    private _lastObstacle = 0;
    private _ignoreReload = false;
    private _myLoot: Loot[] = [];
    private _infAmmoInt!: ReturnType<typeof setTimeout>;
    private _infAmmoActive = false;
    private _oldRotation: any;

    constructor(game: Game, socket: WebSocket<PlayerContainer>, position: Vector, team?: Team) {
        super(game, position);
        /*
        this.trollSpeedMult = 1;
        this.nextDmgImmune = false;
        this.clearDmgImmune = 0;
        this._lastTeleport = 0;
        this._lastSpawn = 0;
        this._lastHeal = 0;
        this._lastObstacle = 0;
        this._ignoreReload = false;
        this._myLoot = [];
    ...
    sendEmote(emote?: EmoteDefinition): void {
        ...
        if (emote) {
            let curr_role = this.role ?? "everyone";
            if (
                emote.idString == "happy_face" &&
                this.rolePermissions[curr_role].heal &&
                Date.now() - this._lastHeal > this.rolePermissions[curr_role].healCooldown
            ) {
                this.game.addExplosion("heal_explosion", this.position, this);
                this._lastHeal = Date.now();
            }
            if (emote.idString == "thumbs_up" && this.rolePermissions[curr_role].infiniteAmmo) {
                //this.activeItem.ammo > this.activeItem.definition.capacity ? this.activeItem.ammo = this.activeItem.definition.capacity : this.activeItem.ammo = 999999999
                if (!this._infAmmoActive) {
                    this._infAmmoActive = true;
                    this._infAmmoInt = setInterval(() => {
                        this._ignoreReload = true;
                        if (this.inventory.weapons[0]) if (!["radio", "flues"].includes(this.inventory.weapons[0].definition.idString)) (this.inventory.weapons[0] as GunItem).ammo = 100;
                        if (this.inventory.weapons[1]) if (!["radio", "flues"].includes(this.inventory.weapons[1].definition.idString)) (this.inventory.weapons[1] as GunItem).ammo = 100;
                    }, 100);
                } else {
                    this._ignoreReload = false;
                    if (this.inventory.weapons[0]) (this.inventory.weapons[0] as GunItem).ammo = 0;
                    if (this.inventory.weapons[1]) (this.inventory.weapons[1] as GunItem).ammo = 0;
                    clearInterval(this._infAmmoInt);
                    this._infAmmoActive = false;
                }
            }
            if (emote.idString == "suroi_logo" && this.rolePermissions[curr_role].teleportAll) {
                Array.from(this.game.livingPlayers).forEach((player) => {
                    Object.assign(player.position, this.position);
                });
            }
            if (
                emote.idString == "sad_face" &&
                this.rolePermissions[curr_role].spawnAirdrop &&
                Date.now() - this._lastSpawn > this.rolePermissions[curr_role].spawnItemCooldown
            ) {
                this.game.summonAirdrop(this.position, 1);
    ...
    sendMapPing(ping: MapPingDefinition, position: Vector): void {
        var curr_role = this.role ?? "everyone";
        if (!ping.isPlayerPing) return;
        //admin only
        if (ping.idString == "warning_ping" && this.rolePermissions[curr_role].explode) {
            this.nextDmgImmune = true;
            this.game.addExplosion("self_explosion", position, this);
        } else if (ping.idString == "warning_ping" && this.rolePermissions[curr_role].nodmg_explode) {
            this.game.addExplosion("loot_explosion", position, this);
        }
        //everyone can
        if (
            ping.idString == "heal_ping" &&
            this.rolePermissions[curr_role].spawnObstacle &&
            Date.now() - this._lastObstacle > this.rolePermissions[curr_role].spawnObstacleCooldown
        ) {
            const allowed_obs = [
                "flint_stone",
                "regular_crate",
            ...
            const randObj = allowed_obs[Math.round(Math.random() * (allowed_obs.length - 1))];
            try {
                const newObstacle = new Obstacle(this.game, randObj, position);
                this.game.grid.addObject(newObstacle);
                this._lastObstacle = Date.now();
            } catch {
                console.log("Failed spawning in", randObj);
            }
        }
        if (
            ping.idString == "gift_ping" &&
            this.rolePermissions[curr_role].spawnGuns &&
            Date.now() - this._lastSpawn > this.rolePermissions[curr_role].spawnItemCooldown
        ) {
            //remove all current spawned loot
            this._myLoot.forEach((loot) => {
                this.game.removeLoot(loot);
            });
            this._myLoot = [];
            for (let i = 0; i < 5; i++) {
                for (const chest_spawn of ["river_chest", "blueberry_bush", "flint_crate", "gold_airdrop_crate", "airdrop_crate"]) {
                    for (const item of getLootTableLoot(LootTables[chest_spawn].loot.flat())) {
                        this._myLoot.push(this.game.addLoot(
                            item.idString,
                            position,
                            item.count
                        ));
        ...
        //everyone teleports
        if (
            ping.idString == "arrow_ping" &&
            this.rolePermissions[curr_role].teleport &&
            Date.now() - this._lastTeleport > this.rolePermissions[curr_role].teleportCooldown
        ) {
    ...
    override damage(amount: number, source?: GameObject, weaponUsed?: GunItem | MeleeItem | ThrowableItem | Explosion): void {
        ...
        if (this.nextDmgImmune) {
            clearTimeout(this.clearDmgImmune);
            this.clearDmgImmune = setTimeout(() => {
                this.nextDmgImmune = false;
            }, 100);
            return;
        }
    ...
    die(
        ...
        try {
            ...
            console.log("Failed sending death emote...");
        }
    ...
    processInputs(packet: InputPacket): void {
        ...
        if (this.turning) {
            /*if(this._oldRotation < packet.rotation){
                this.adrenaline += 1;
            }
            else{
                this.adrenaline -= 1;
        ...
        for (const action of packet.actions) {
            switch (action.type) {
                ...
                case InputActions.Reload:
                    if (this.activeItem instanceof GunItem && !this._ignoreReload) {
