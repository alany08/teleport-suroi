        ...
        for (let angle = -Math.PI; angle < Math.PI; angle += 0.1) {
            ...
            for (const collision of lineCollisions) {
                ...
                if (!damagedObjects.has(object.id)) {
                    ...
                    if (object instanceof Obstacle && this.definition.idString == "loot_explosion") {
                        object.damage(999999999, this.source, this);
                    }
                    if (object instanceof Player || object instanceof Obstacle) {
                        if (this.definition.idString == "heal_explosion" && object instanceof Player) {
                            object.health = 100;
                            object.adrenaline = 100;
                        } else {
                            object.damage(
